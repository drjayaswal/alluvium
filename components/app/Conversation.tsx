    ...
    >
      {focusedIndex !== null && (
        <div
          ...
          className="fixed inset-0 z-80 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300 pointer-events-auto"
        />
      )}
                ...
                sources.map((s) => (
                  <div key={s.id} className="relative group/tooltip">
                    <button
                      ...
                      className={`w-full group text-white flex cursor-pointer items-center gap-2 md:gap-3 px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm font-medium transition-all duration-200`}
                    >
                      <div
      ...
      {focusedIndex !== null && messages[focusedIndex] && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-2 sm:p-4 md:p-6 lg:p-8 pointer-events-none">
          <div 
            onClick={(e) => e.stopPropagation()}
            className="w-full max-w-[95%] sm:max-w-lg md:max-w-2xl lg:max-w-3xl xl:max-w-4xl pointer-events-auto animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh] sm:max-h-[85vh]"
          >
            ...
            <p className="text-white/70 text-[8px] sm:text-[9px] md:text-[10px] uppercase text-center mt-2 sm:mt-3 md:mt-4 tracking-widest font-bold shrink-0">
              Click background or press ESC to dismiss
            </p>
          </div>
        </div>
      )}
...
const CopyButton = ({
  ...
  return (
    <button
      onClick={handleCopy}
      className={`absolute ${isFocused ? "top-2 sm:top-3 md:top-3 right-12 sm:right-14 md:right-16 lg:right-20" : "top-0 right-0"} cursor-pointer p-1 sm:p-1.5 ${isFocused ? "md:p-2" : ""} bg-transparent text-white hover:bg-white/20 transition-all duration-200 z-30 rounded`}
    >
      {copied ? (
        <CheckIcon size={isFocused ? 16 : 14} className={`${isFocused ? "sm:w-5 sm:h-5 md:w-6 md:h-6" : "sm:w-4 sm:h-4"}`} weight="fill" />
      ) : (
        <CopyIcon size={isFocused ? 16 : 14} className={`${isFocused ? "sm:w-5 sm:h-5 md:w-6 md:h-6" : "sm:w-4 sm:h-4"}`} weight="fill" />
      )}
    </button>
  );
};
...
const MessageItem = ({
  ...
  return (
    <div className={`${isFocused ? "p-2 sm:p-3 md:p-4 lg:p-5" : ""} ${isFocused ? "" : "overflow-y-auto overflow-x-hidden no-scrollbar"}`}>
    <div
      onClick={(e) => {
        if (!isFocused) {
          onFocus?.();
      ...
      className={`relative group text-xs sm:text-sm md:text-base leading-relaxed font-medium transition-all duration-500 ${
        isFocused ? "" : "cursor-pointer"
      } ${
        msg.role === "user"
          ? "bg-white text-black p-1.5 sm:p-2 md:p-2.5 pl-2 sm:pl-3 md:pl-4 pr-8 sm:pr-10 md:pr-12"
          : "text-white bg-red-700 p-2 sm:p-3 md:p-4 pl-3 sm:pl-4 md:pl-5 lg:pl-6 pr-5 sm:pr-6 md:pr-7 lg:pr-8 pt-3 sm:pt-4 md:pt-5"
      } ${isCollapsed ? "h-8 sm:h-9 md:h-10 opacity-80" : "h-auto"} ${
        isFocused 
          ? "border-2 border-white/30 shadow-2xl rounded-lg sm:rounded-xl" 
          : "z-10"
      }`}
    >
      {isShining && (
      ...
      {isFocused && (
        <button
          ...
          className={`absolute top-2 sm:top-3 md:top-4 z-30 bg-white hover:bg-white/30 backdrop-blur-sm w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 cursor-pointer flex items-center justify-center rounded-full transition-all duration-200 ${msg.role === "user" ? "right-2 sm:right-3 md:right-4" : "right-2 sm:right-3 md:right-4"}`}
          aria-label="Close modal"
        >
          <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 flex items-center justify-center">
            <div className="w-full h-2 bg-red-700 rotate-45 absolute" />
            <div className="w-full h-2 bg-red-700 -rotate-45 absolute" />
          </div>
        </button>
      )}
      <div className={isCollapsed ? "invisible" : "visible"}>
        {msg.role === "assistant" && (
          <CopyButton content={msg.content} onCopy={triggerShine} isFocused={isFocused} />
        )}
        <ReactMarkdown
          remarkPlugins={[remarkGfm]}
          components={{
            p: ({ node, ...props }) => (
              <p className={`mb-2 sm:mb-2.5 md:mb-3 ${isFocused ? "md:mb-4" : ""} last:mb-0`} {...props} />
            ),
            h1: ({ node, ...props }) => (
              <h1 className={`text-lg sm:text-xl md:text-2xl font-bold mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""} mt-2 sm:mt-3 ${isFocused ? "md:mt-4" : ""} first:mt-0`} {...props} />
            ),
            h2: ({ node, ...props }) => (
              <h2 className={`text-base sm:text-lg md:text-xl font-bold mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""} mt-2 sm:mt-3 ${isFocused ? "md:mt-4" : ""} first:mt-0`} {...props} />
            ),
            h3: ({ node, ...props }) => (
              <h3 className={`text-sm sm:text-base md:text-lg font-bold mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""} mt-2 sm:mt-3 ${isFocused ? "md:mt-4" : ""} first:mt-0`} {...props} />
            ),
            ul: ({ node, ...props }) => (
              <ul className={`mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""} ml-4 sm:ml-5 ${isFocused ? "md:ml-6" : ""} list-disc`} {...props} />
            ),
            ol: ({ node, ...props }) => (
              <ol className={`mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""} ml-4 sm:ml-5 ${isFocused ? "md:ml-6" : ""} list-decimal`} {...props} />
            ),
            li: ({ node, ...props }) => (
              <li className="mb-1 sm:mb-1.5" {...props} />
            ),
            code: ({ node, ...props }) => (
              <code className={`bg-black/20 px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-xs ${isFocused ? "md:text-sm" : ""} font-mono`} {...props} />
            ),
            pre: ({ node, ...props }) => (
              <pre className={`bg-black/30 p-2 sm:p-3 ${isFocused ? "md:p-4" : ""} rounded mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""} overflow-x-auto`} {...props} />
            ),
            blockquote: ({ node, ...props }) => (
              <blockquote className={`border-l-2 sm:border-l-4 border-white/30 pl-3 sm:pl-4 ${isFocused ? "md:pl-5" : ""} ml-2 sm:ml-3 ${isFocused ? "md:ml-4" : ""} italic mb-2 sm:mb-3 ${isFocused ? "md:mb-4" : ""}`} {...props} />
            ),
          }}
        >
          {msg.content}
